...
export default function EditAchievementForm({ data }) {
  const fileUpload = useRef(null);
  const { data: session } = useSession();
  const userId = session?.user?.userId;
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [mediaFiles, setMediaFiles] = useState(JSON.parse(data.MEDIA).media);
  ...
  const [formData, setFormData] = useState({
    ...
    mainMediaType: data.MAIN_MEDIA_TYPE,
    mainMediaVideo: data.MAIN_MEDIA_VIDEO,
    mainMediaImage: data.MAIN_MEDIA_IMAGE,
  });
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-4 h-4 z-40 flex items-center justify-center rotate-45 absolute -top-1 -right-1 rounded-full bg-red-500"
                  >
                    <Plus size={10} />
                  </button>
                  {item.type === "V" ? (
