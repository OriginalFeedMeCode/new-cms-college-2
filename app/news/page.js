...
export default async function ViewAllNewsPage() {
  // Fetching data directly within the function
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/get-news`,
    { cache: "no-store" }
  );
          ...
          {news.map((item, index) => (
            <tr key={index} className="hover:bg-gray-50">
              ...
              <td className="max-w-96">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 w-max items-center">
                  <Link target="_blank" className="btn" href={item.SOURCE_URL}>
                    View Source
                  </Link>
                  <Link className="btn" href={`/news/${item.ID}`}>
                    View News
                  </Link>
                  <DeleteNewsButton
                    postId={item.ID}
