...
export default async (req, res) => {
  ...
  try {
    ...
    const updateQuery = `
      UPDATE ACHIEVEMENT SET STATUS = ?, APPROVED_BY = ? WHERE ID = ?
    `;
    ...
    return res.status(200).json({
      status: "success",
      message: "Achievement status and approval updated successfully",
    });
  } catch (error) {
    console.error("Error while updating achievement status:", error);
